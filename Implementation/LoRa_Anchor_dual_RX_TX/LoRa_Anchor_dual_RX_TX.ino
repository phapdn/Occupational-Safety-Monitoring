...
// ========== PACKET (75 bytes - khớp với Full_System_DualCore) ==========
#pragma pack(push, 1)
struct DataPacket {
  ...
  float spo2;             // 4 bytes
  float range_A0;         // 4 bytes
  float range_A1;         // 4 bytes
  float range_TAG2;       // 4 bytes
  float range_A2;         // 4 bytes
  float distance_A1;      // 4 bytes
  float distance_A2;      // 4 bytes
  uint8_t uwbReady;       // 1 byte
...
void rxTask(void *pvParameters) {
  ...
  for(;;) {
    ...
    if (packetSize > 0) {
      ...
      packetsReceived++;
      Serial.printf("✅ RX | MAC:%012llX | %ddBm | T:%.1f°C | Bat:%.0f%% | HR:%.0f | GPS:%.4f,%.4f",
                    pkt.mac, rssi, pkt.bodyTemp, pkt.batteryPercent, 
                    pkt.heartRate, pkt.latitude, pkt.longitude);
...
void setup() {
  Serial.begin(115200);
  delay(2000);
  Serial.println("\n🚀 DUAL-CORE RELAY (PURE REGISTER TX)");
  Serial.println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  ...
  uint32_t uwb = readDeviceID();
  Serial.printf("🔸 UWB: 0x%08X\n", uwb);
