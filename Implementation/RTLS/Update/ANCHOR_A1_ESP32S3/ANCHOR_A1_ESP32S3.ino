...
void setup(){
  ...
  delay(200);
  Serial.println("========================================");
  Serial.println("ANCHOR A1 - CALIBRATION MODE");
  Serial.println("========================================");
  ...
  DW1000Ng::setDeviceAddress(0xDDCC5BD5);
  DW1000Ng::setAntennaDelay(16500);
  DW1000Ng::setTXPower(0x1F1F1F1F);

  Serial.println("✅ A1 Ready - Will send 5 POLL for calibration");
  delay(2000);
}
...
void loop(){
  if(current_mode == CALIBRATION){
    if(calib_sent >= 5){
      Serial.println("========================================");
      Serial.println("✅ A1: Sent 5 POLL for calibration");
      Serial.println("🔄 A1 → NORMAL MODE");
      Serial.println("========================================");
      current_mode = NORMAL;
      delay(1000);
      return;
    ...
    byte tmp[5];
    DW1000NgUtils::writeValueToBytes(tmp, TS1, 5);
    memcpy(&rpt[4], tmp, 5);
    DW1000NgUtils::writeValueToBytes(tmp, TS4, 5);
    memcpy(&rpt[9], tmp, 5);
    DW1000NgUtils::writeValueToBytes(tmp, TS5, 5);
    memcpy(&rpt[14], tmp, 5);
